...
// get variables that client chose by using local storage, and multiple by 60 to convert to minutes 
function getTimeElements(){
  localStorage.setItem("work_time", 60*document.getElementById("workInputMinute").value);
  localStorage.setItem("short_break_time", 60*document.getElementById("shortInputMinute").value);
  localStorage.setItem("long_break_time", 60*document.getElementById("longInputMinute").value);
...
function onTimesUp(last) {
  stopTimer();
  turnOff(last);
  playSound('../static/ringtone/time_up.mp3');
}
